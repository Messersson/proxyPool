...
配置文件 `setting.py` 位于项目的主目录下，配置主要分为：**服务配置**、**数据库配置**、**采集配置**、**校验配置**、**调度配置**。
...
可选 API 鉴权 Token。为空表示不鉴权；设置后访问业务接口需携带：

- 请求头 `X-API-Token: <token>`，或
- 请求头 `Authorization: Bearer <token>`，或
- Query 参数 `?token=<token>`

也可用环境变量 `API_TOKEN` 覆盖。
...
代理源黑名单。同时支持 `name` 与类名，列表中的代理源不会被加载，即使 `enabled=True`。适用于临时禁用某个代理源而不修改其源文件。

```python
PROXY_FETCHER_EXCLUDE = [
    "freevpnnode",           # 按 name 禁用
    "KuaidailiFetcher",      # 按类名禁用
]
```
...
环境变量解析支持：`true/false/1/0/yes/no/on/off`（大小写不敏感）。
...
同一任务允许的最大并行实例数。建议保持 `1`，避免任务堆积。

- 默认值：`1`

## 网页配置中心

访问 `http://127.0.0.1:5010/admin/config` 可在网页表单中修改配置并保存。

- 保存文件：`data/runtime_config.json`
- 保存后立即热加载（`ConfigHandler.reload()`）
- 优先级：环境变量 > 网页保存配置 > `setting.py`
- 若设置了 `API_TOKEN`，打开管理页接口时需携带 Token
- 标记“需重启”的项（如监听端口、调度间隔）会写入文件，但完全生效需重启对应进程

也可通过 API：

```bash
# 读取
curl http://127.0.0.1:5010/admin/config/api

# 保存
curl -X POST http://127.0.0.1:5010/admin/config/api \
  -H "Content-Type: application/json" \
  -d "{\"MAX_FAIL_COUNT\": 2, \"HTTP_URL\": \"http://httpbin.org\"}"
```
